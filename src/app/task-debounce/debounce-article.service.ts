...
export class DebounceArticleService {
  ...
  articlesFromSearch$ = combineLatest([
    this.search$,
    this.fetchArticles(100),
  ]).pipe(
    map(([s, as]) =>
      as.filter(
        (a) =>
  ...
  constructor(private apiClient: ApiService, private http: HttpClient) {}

  private fetchArticles(count: number, skip?: number): Observable<Article[]> {
    skip = skip || 0;
    return (
      this.http
        .get<{ articles: Article[] }>(`/assets/articles.json`)
